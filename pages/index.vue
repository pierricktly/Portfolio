<template>
  <div class="flex min-h-[calc(100vh-150px)] flex-col justify-center">
    <div id="name" class="space-y-5 font-bold lg:w-3/4 lg:space-y-8 2xl:space-y-12">
      <p class="text-primary md:text-xl xl:text-[1.2vw]">{{ $t('hello') }}</p>
      <h1 class="text-4xl font-bold xl:text-[3vw]">Pierrick Taïly</h1>
      <p
        class="text-lg font-semibold leading-10 md:text-xl lg:w-3/4 lg:leading-10 xl:text-[1vw]"
      >
        {{ $t('description') }}
      </p>

      <div class="flex gap-5 text-xs font-semibold lg:text-base">
        <button
          @click="copyEmail()"
          class="flex items-center gap-2 bg-primary px-5 py-2 text-secondary hover:bg-primary/90"
        >
